...
void decodePacket(struct TlvDecodeDescriptor *descriptor, const uint8_t *data)
{
    ...
    case TotalPacketLen:
        ...
        if (descriptor->ofst >= sizeof(descriptor->frameHeader.numTotalBytes))
        { // 4 bytes
...
// CRC32
uint32_t CRC32_BYTES2IGNORE = 16; // first 16 bytes of header
...
uint32_t CRC32(uint8_t *pData, uint32_t unLength)
{
    ...
    nReg = 0xFFFFFFFF;
    for (n = 0x00; n < unLength; n++)
    {
        nReg ^= (uint32_t)pData[n];
        for (i = 0x00; i < 4; i++)
        {
            nReg <<= 8;
            nReg ^= CRC32_TABLE[(uint8_t)((nReg >> 24) & 0xff)];
        }
    }
    return nReg;
}